...
func (c *Cms) mailNtfnProposalNew(token, name, username string, emails []string) error {
...
func (p *Cms) mailNtfnProposalEdit(token string, version uint32, name, username string, emails []string) error {
	route := strings.Replace(guiRouteRecordDetails, "{token}", token, 1)
...
func (p *Cms) mailNtfnProposalSetStatus(token, name string, status rcv1.RecordStatusT, emails []string) error {
	route := strings.Replace(guiRouteRecordDetails, "{token}", token, 1)
	u, err := url.Parse(p.cfg.WebServerAddress + route)
	if err != nil {
		return err
...
func (p *Cms) mailNtfnProposalSetStatusToAuthor(token, name string, status rcv1.RecordStatusT, reason, authorEmail string) error {
	route := strings.Replace(guiRouteRecordDetails, "{token}", token, 1)
	u, err := url.Parse(p.cfg.WebServerAddress + route)
	if err != nil {
		return err
...
func (p *Cms) mailNtfnCommentNewToProposalAuthor(token string, commentID uint32, commentUsername, proposalName, proposalAuthorEmail string) error {
...
func (p *Cms) mailNtfnCommentReply(token string, commentID uint32, commentUsername, proposalName, parentAuthorEmail string) error {
	cid := strconv.FormatUint(uint64(commentID), 10)
	route := strings.Replace(guiRouteRecordComment, "{token}", token, 1)
	route = strings.Replace(route, "{id}", cid, 1)
