...
// GetDCC request finds a DCC with a matching token.
type GetDCC struct {
	Token string `json:"token"` // Token of requested DCC
}

// GetDCCReply returns the DCC if found.
type GetDCCReply struct {
	DCC DCCRecord `json:"dcc"` // DCCRecord of requested token
}

// GetDCCs request finds all DCCs that have matching status (if used).
type GetDCCs struct {
	Status DCCTypeT `json:"status"` // Return all DCCs of this status
}
