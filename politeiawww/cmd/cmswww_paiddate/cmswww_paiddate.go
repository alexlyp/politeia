...
func _main() error {
	...
	paid, err := cmsDB.InvoicesByStatus(int(cms.InvoiceStatusPaid))
	if err != nil {
		return fmt.Errorf("payments by status: %v", err)
	}
	for _, inv := range paid {
		invoice, err := cacheDB.Record(inv.Token)
		if err != nil {
			fmt.Printf("couldn't get invoice for paid date check: %v, %v\n",
				inv.Token, err)
			continue
		}
		found := false
		var paidStatusChange cmsdatabase.InvoiceChange
		for _, v := range invoice.Metadata {
			if v.ID == mdstream.IDInvoiceStatusChange {

				sc, err := mdstream.DecodeInvoiceStatusChange([]byte(v.Payload))
				if err != nil {
					fmt.Printf("could not decode metadata '%v' token '%v': %v\n",
						v, inv.Token, err)
					break
				}
				for _, s := range sc {
					if s.NewStatus == cms.InvoiceStatusPaid && s.Reason != "Tool updated date!" {
						paidStatusChange = cmsdatabase.InvoiceChange{
							AdminPublicKey: s.AdminPublicKey,
							NewStatus:      s.NewStatus,
							Reason:         s.Reason,
							Timestamp:      s.Timestamp,
						}
					} else if s.NewStatus == cms.InvoiceStatusPaid && s.Reason == "Tool updated date!" {
						found = true
						break
					}
		...
		if found {
			fmt.Printf("Invoice status change has already been updated for %v\n", inv.Token)
			continue
		}
		txIDs := strings.Split(inv.Payments.TxIDs, ",")
		if len(txIDs) < 1 {
			continue
		}
		earliestTxTime := time.Now()
		for _, txID := range txIDs {
			if txID == "" {
				continue
			}
			txDetails, err := util.FetchTx(inv.Payments.Address, txID)
			if err != nil {
				fmt.Printf("error fetching tx: %v %v\n", txID, err)
				continue
			}
			if time.Unix(txDetails.Timestamp, 0).Before(earliestTxTime) {
				earliestTxTime = time.Unix(txDetails.Timestamp, 0)
			}
		}

		//fmt.Printf("%v earliestTxTime: %v  last status time: %v\n", inv.Token, earliestTxTime.String(), time.Unix(paidStatusChange.Timestamp, 0))
		if earliestTxTime.After(time.Unix(paidStatusChange.Timestamp, 0)) {
			fmt.Printf("%v tx time is AFTER the status change %v %v\n", inv.Token, earliestTxTime, time.Unix(paidStatusChange.Timestamp, 0))
		} else {
			fmt.Printf("%v tx time is BEFORE the status change %v %v\n", inv.Token, earliestTxTime, time.Unix(paidStatusChange.Timestamp, 0))
		}
		/*
			// If the paid status change time of the metadata is AFTER the earilest time plus 1 day, that means
			if time.Unix(paidStatusChange.Timestamp, 0).Add(time.Hour * 24).After(earliestTxTime) {
